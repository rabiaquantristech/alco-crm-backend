...
// ✅ NEW — old user account setup karne ke liye
router.post("/complete-setup", protect, authController.completeAccountSetup);
...
router.get(
  ...
  (req, res) => {
    try {
      ...
      if (!frontend) throw new Error("FRONTEND_URL is missing");

      const userData = encodeURIComponent(JSON.stringify({ _id: req.user._id, name: req.user.name, email: req.user.email }));
      return res.redirect(`${frontend}/auth/callback?token=${token}&user=${userData}`);
    } catch (error) {
...
router.get("/linkedin/callback", async (req, res) => {
  ...
  try {
    const tokenRes = await axios.post(
      ...
      new URLSearchParams({ grant_type: "authorization_code", code, redirect_uri: `${process.env.BACKEND_BASE_URL}/api/auth/linkedin/callback`, client_id: process.env.LINKEDIN_CLIENT_ID, client_secret: process.env.LINKEDIN_CLIENT_SECRET }),
    ...
    if (!user) {
      user = await User.create({ name, email, isVerified: true, password: "oauth_linkedin" });
    }
...
// ─── DASHBOARD ROUTES ─────────────────────────────────────────
router.get("/admin-dashboard", protect, authorize("super_admin", "admin"), (req, res) => res.json({ message: "Welcome Admin" }));
router.get("/manager-dashboard", protect, authorize("sales_manager", "sales_rep"), (req, res) => res.json({ message: "Welcome Manager" }));
router.get("/rep-dashboard", protect, authorize("sales_manager", "sales_rep"), (req, res) => res.json({ message: "Welcome Sales Rep" }));
router.get("/support-dashboard", protect, authorize("support"), (req, res) => res.json({ message: "Welcome Support" }));
router.get("/finance-dashboard", protect, authorize("finance_manager"), (req, res) => res.json({ message: "Welcome Finance Manager" }));
router.get("/user-dashboard", protect, authorize("user"), (req, res) => res.json({ message: "Welcome User" }));
...
// // ================= DASHBOARD ROUTES =================
// router.get("/admin-dashboard", protect, authorize("super_admin", "admin"), (req, res) => {
//   res.json({ message: "Welcome Admin" });
// });

// router.get("/manager-dashboard", protect, authorize("sales_manager", "sales_rep"), (req, res) => {
//   res.json({ message: "Welcome Manager" });
// });
